...
import { Activity, useBooking } from '../context/BookingContext';
...
export function Summary() {
  ...
  const handleCancelBooking = (activityId: string) => {
    ...
    console.log('Cancelling booking for activity:', activityId);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">My Activity Calendar</h1>
                ...
                return (
                  <div
                    ...
                    className={`aspect-square border rounded-lg p-2 transition-all cursor-pointer ${
                      isToday ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'
                    }`}
                    onMouseEnter={(e) => activity && handleActivityHover(activity, e)}
                    onMouseLeave={() => handleActivityHover(null)}
                    onMouseMove={(e) => activity && setHoverPosition({ x: e.clientX, y: e.clientY })}
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-center space-y-2">
                <div className="text-sm font-medium text-gray-700 mb-2">Community Impact</div>
                <div className="text-xs space-y-1">
                  <div className="flex justify-between">
                    <span className="text-gray-600">Direct Local Partner:</span>
                    <span className="font-semibold text-blue-600">
                      {bookedActivities.filter(a => a.communityImpact === 'Direct Local Partner').length}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Internal Support:</span>
                    <span className="font-semibold text-blue-600">
                      {bookedActivities.filter(a => a.communityImpact === 'Internal Community Support').length}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">No Direct Link:</span>
                    <span className="font-semibold text-blue-600">
                      {bookedActivities.filter(a => a.communityImpact === 'No Direct Community Link').length}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="text-xs space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-emerald-600">
                      {bookedActivities.filter(a => a.environmentalImpact === 'High').length}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Medium:</span>
                    <span className="font-semibold text-emerald-600">
                      {bookedActivities.filter(a => a.environmentalImpact === 'Medium').length}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Low:</span>
                    <span className="font-semibold text-emerald-600">
                      {bookedActivities.filter(a => a.environmentalImpact === 'Low').length}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                ...
                {selectedActivity.bookingDate && (
                  <div className="text-base font-semibold text-gray-900">
                    {new Date(selectedActivity.bookingDate).toLocaleDateString('en-US', { 
                      weekday: 'long', 
                      year: 'numeric', 
                      month: 'long', 
                      day: 'numeric' 
                ...
                <div className="text-sm text-gray-600 mt-1">{selectedActivity.bookingTime}</div>
              </div>
                          ...
                          <div className="space-y-2">
                            <div>
                              ...
                              <div className="flex items-center gap-4 text-xs text-gray-500">
                                ...
                                <span>👥 {activity.communityImpact}</span>
                                <span>🌿 {activity.environmentalImpact}</span>
                              </div>
                            </div>
                            <Button
                              variant="destructive"
